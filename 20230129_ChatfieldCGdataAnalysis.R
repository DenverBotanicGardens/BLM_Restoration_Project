...
ARFR <- read.csv(file="Chatfield/20230228_ChatfieldData_ARFR.csv", sep=",", header=TRUE, dec=".")
...
#If OrigPltSurv_20220527 = 0 and plt not replaced (N),
#ignore subsequent data for this plant, i.e. future survival should be NA and not 0
#ARFR$Replaced_YorN_20220531[ARFR$Replaced_YorN_20220531==""] <- "N" 
ARFR <- ARFR[ARFR$OrigPltSurvival_20220527==1 | (ARFR$OrigPltSurvival_20220527==0 & ARFR$Replaced_YorN_20220531=="Y"),]

#Don't use OrigPltSurv_20220527 data in days to mort or other field analyses,
#this surv may not correspond to plt names in Source/Pop col (may correspond to orig planted or assigned)
#For ARFR, some that were not dead (had an initial sz measure) were replaced. In this case, don't use length_527;
#so if length_527 is not NA and Replaced = Y, then ignore length_527 as it doesn't correspond to plt names in source column
#If not replaced, but died before planting, don't use subsequent surv data;
#but if died after planting and not replaced, use subsequent surv data.
#Check that once zero in surv on 6/22 or later, stays zero (cannot become 1 later)
...
## COMBINE DATA TYPES --------------------------------------------
#ERBR <- left_join(ERNA, ERNA.SdZn, by="Source")
ARFR <- left_join(ARFR, ARFR.SdZn, by="Source")
ARFR <- left_join(ARFR, ARFR.ppt, by="Source")
ARFR <- left_join(ARFR, ARFR.tmin, by="Source")
...
## ** UPDATE GROWTH FORMULA ******
boxplot((Length_cm_20220726/Length_cm_20220622) ~ Source, data=ARFR, names=ARFR.SdZn$PopAbbrev,
        xlab="Population", ylab="Plant growth", cex.lab=1.5, ylim=c(0,14),
        cex.axis=1.1, main="Artemisia frigida", cex.main=1.5, col=ARFR.SdZn$SdZnCol)


boxplot(AGB_MinusBag ~ Source, data=ARFR,
...
ARFR.bmMod <- lmer(log(AGB_minusBag) ~ Source + (1|Block), data = ARFR)
...
ARFR.ht.cv <- ARFR %>% group_by(Source) %>% summarise(Height_CV=cv(Length_cm_20220726, na.rm=TRUE))
ARFR.gr.cv <- ARFR %>% group_by(Source) %>% summarise(Growth_CV=cv(Length_cm_20220726/Length_cm_20220622, na.rm=TRUE))
ARFR.bm.cv <- ARFR %>% group_by(Source) %>% summarise(Biomass_CV=cv(AGB_MinusBag, na.rm=TRUE))
